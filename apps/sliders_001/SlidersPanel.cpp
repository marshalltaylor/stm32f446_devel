...
static void crapDelay( uint32_t msInput );
...
SlidersPanel::SlidersPanel( void )
{
	...
	sw3Down.setHardware(new ArduinoDigitalIn( D31 ), 1);
	sw4Down.setHardware(new ArduinoDigitalIn( D32 ), 1);
	sw5Down.setHardware(new ArduinoDigitalIn( D33 ), 1);
	add( &sw1Down );
	add( &sw2Down );
	add( &sw3Down );
	add( &sw4Down );
...
//			pattern.printPattern();
	...
	{
		localPrintf("controlNoteMixer inputNoteOnList\n");
		controlNoteMixer.inputNoteOnList.printfMicroLL();
		localPrintf("controlNoteMixer outputNoteBuffer\n");
		controlNoteMixer.outputNoteBuffer.printfMicroLL();
		localPrintf("outputNoteMixer keyboardInputNoteOnList\n");
		outputNoteMixer.keyboardInputNoteOnList.printfMicroLL();
		localPrintf("outputNoteMixer playerInputNoteOnList\n");
		outputNoteMixer.playerInputNoteOnList.printfMicroLL();
		localPrintf("outputNoteMixer outputNoteBuffer\n");
